...
if (!isset($_SESSION['idadmin'])) {
    ...
    if (isset($_GET['detail_book_request'])) {
        $book_id = $_GET['detail_book_request'];
        $student_id = $_GET['student_request'];

        $get_book = "select * from bookofstudent where BOOK_ID='$book_id' and STUDENT_ID='$student_id' group by BOOK_ID";
    ...
    <div class="row">
        <div class="col-lg-12">
            <ol class="breadcrumb">
                <li>
                    <i class="fa fa-dashboard"></i> Dashboard / View Request Create Book / Edit Book
                </li>
            </ol>
        </div>
    </div>
    ...
    if (isset($_POST['update'])) {
        ...
        $img1 = $_POST['img'];
        if ($img1 == '') {
            echo $img1 = $book_image;
        }
        $update_book = "update bookofstudent set BOOK_NAME='$book_name',BOOK_TYPE='$book_type',DESCRIPTION='$book_desc',BOOK_IMAGE='$img1',BOOK_REQUEST_CREATE=1 where BOOK_ID='$book_id' and STUDENT_ID='$student_id'";
        ...
        if ($run_update_book) {
            ...
            echo "<script>window.open('index.php?view_request_cb','_self')</script>";
        }
    }
